# Demo App - Slide Decks Documentation

## reveal.js docs below

## What is this?

A simple fork of the reveal.js slide deck application wrapped with some Jenkins and OpenShift manifests.  Just used primarily as a demonstration for simple BuildConfigs of a Node.js application.
...
2. Clone this repository
   ```sh
   $ git clone https://github.com/kenmoini/demo-app-slide-deck.git
   ```

3. Navigate to the reveal.js folder
   ```sh
   $ cd demo-app-slide-deck
   ```
...
1. Apply/Import the ```openshift/cicd-config-map-nodejs.yaml``` ConfigMap for the NodeJS Jenkins Agent
2. Apply/Import the ```openshift/openshift-pipeline.yaml``` BuildConfig to import the Pipeline
3. Run the Pipeline
4. ??????
5. PROFIT!!!!!1
...
4. (Optional) Use a crontab entry with the time of **@reboot** to start the npm server with `npm server -- --prefix=/path/to/demo-app-slide-deck`
